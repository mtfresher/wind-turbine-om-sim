...
def main():
    ...
    wind_speeds, wind_dirs = wind_field_manager.simulate(steps=36000)
    ...
    bearing_temperatures = bearing_temp_simulator.simulate(minutes=100)

    # 风机轴承振动模拟
    bearing_vibration_simulator = BearingVibrationSimulator()
    bearing_vibrations = bearing_vibration_simulator.simulate(steps=100)

    # 可视化结果
    plt.figure(figsize=(12, 8))

    plt.subplot(3, 2, 1)
    plt.plot(wind_speeds[-360:], label='Wind Speed (m/s)')
